pipeline {
    agent any

    environment {
        // Replace this with your EKS Service Account token
        K8S_TOKEN = 'eyJhbGciOiJSUzI1NiIsImtpZCI6IjdXM3d2WTNmSXlaZndmUTJKRjhQMktTMi00ZkxfLVRWU0p3NnpOTm00cWcifQ..Z_D1fewE0mhaf6dnAnQn2N8pBqwg9n32Myd-BkDVpVnzSbDvH2s5yNwhcOEpq0jbT0MJnt9NtzxhR-p5YLYdSn5YWOJ4mzFsiNdTPfqGeyviT16hifxtaNuEm7cQBNqr8tnEGFtcvyRBpHITbyq2urbBV50Rz1hbVSt-g98swssfVeO0c_-qQODucnWjrGcmSxHiW8IFTwVP4065999LtQJu1nU-Pm3Xx5M5mpwX2d9MgXpzmj_tNn7_pH6urfmmK5-xkA8D7f-RNpfqzAesfqXNP943C3qOH2CFysfl3ffVTgSChG1iWKWMA4KtICT3t2MCXP36unQI5su_tWPW3Q'
        // Base64 CA of your EKS cluster
        K8S_CA = ''
        K8S_NAMESPACE = 'webapps'
        K8S_SERVER = 'https://9F39F577334FF23706994135261985F2.gr7.ap-south-1.eks.amazonaws.com'
    }

    stages {
        stage('Deploy To Kubernetes') {
            steps {
                withKubeCredentials(
                    kubectlCredentials: [[
                        caCertificate: env.K8S_CA,
                        clusterName: 'EKS-1',
                        contextName: '',
                        credentialsId: 'k8-token',
                        namespace: env.K8S_NAMESPACE,
                        serverUrl: env.K8S_SERVER
                    ]]
                ) {
                    sh "kubectl apply -f deployment-service.yml -n ${env.K8S_NAMESPACE} --token=${env.K8S_TOKEN}"
                }
            }
        }

        stage('Verify Deployment') {
            steps {
                withKubeCredentials(
                    kubectlCredentials: [[
                        caCertificate: env.K8S_CA,
                        clusterName: 'EKS-1',
                        contextName: '',
                        credentialsId: 'k8-token',
                        namespace: env.K8S_NAMESPACE,
                        serverUrl: env.K8S_SERVER
                    ]]
                ) {
                    sh "kubectl get svc -n ${env.K8S_NAMESPACE} --token=${env.K8S_TOKEN}"
                }
            }
        }
    }
}
